...
export const findAllCartUser = async (req, res) => {
   try {
      const cart = await Cart.findOne({ orderBy: req.params.id }).populate("products.product").exec();
...
export const updateCart = async (req, res) => {
   try {
      ...
      let newGrandTotal = 0;
      if (actionId == "remove") {
         existCart.products = existCart.products.filter(item => item.product != product);
      }
